"""Optimized transposed-WGMMA INT2 decode attention for Hopper.

This comparison baseline keeps the gather and 16-byte packed-load path from
...
kernel.  Keeping it separate preserves the original fused implementation as a
side-by-side correctness and performance baseline.
"""
...
@cute.jit
def _gemm_zero_acc(tiled_mma, operand_a, operand_b, accumulator):
...
def flashinfer_cutedsl_decode_attention_fwd_int2_transposed(
    ...
    """Run the optimized transposed-WGMMA stage-1 kernel."""
