"""Register-sourced transposed-WGMMA INT2 decode attention for Hopper.

This file is deliberately not wired into the attention dispatch.  It keeps
the gather + 16-byte packed-load path from the fused FlashInfer-derived
kernel, but changes the two GEMMs to

    K[64, 128] @ Q.T[128, 8] -> S.T[64, 8]
    V.T[128, 64] @ P.T[64, 8] -> O.T[128, 8]

The group-size-1 or group-size-4 query rows occupy the leading columns of the
n8 WGMMA tile.  K is loaded as one coalesced 16-byte vector per lane,
warp-transposed to Hopper's m64k16 register fragment, and dequantized one k16
slice at a time immediately before each RS WGMMA.  K never enters shared
memory.  V and P retain the proven swizzled-shared PV path.
...
__all__ = [
    "can_use_flashinfer_cutedsl_decode_wgmma_rs",
    "flashinfer_cutedsl_decode_attention_fwd_int2_wgmma_rs",
]
...
def _define_decode_kernel(
    ...
    def kernel(
        ...
        while tile_start < split_stop:
            # V keeps the proven coalesced path into shared memory.  Its raw
            # bytes die before the K load, limiting the live register set.
...
def can_use_flashinfer_cutedsl_decode_wgmma_rs(
    q: torch.Tensor,
...
def flashinfer_cutedsl_decode_attention_fwd_int2_wgmma_rs(
    q: torch.Tensor,
    k_buffer: torch.Tensor,
    v_buffer: torch.Tensor,
    k_scales_zeros: torch.Tensor,
    ...
    """Run the register-sourced transposed-WGMMA stage-1 kernel."""

    if not can_use_flashinfer_cutedsl_decode_wgmma_rs(
        q, k_buffer, v_buffer, k_scales_zeros, v_scales_zeros
    ):
        raise ValueError(
