"""Fully register-sourced WGMMA INT2 decode attention for Hopper.

This implementation keeps the gather + 16-byte packed-load path from
``flashinfer_cutedsl_int2_decode`` and changes the two GEMMs to
...
Packed V occupies only 2 KiB of shared memory and is dequantized directly into
the native PV register fragment instead of materializing decoded BF16 V.

The public entry point has the same stage-1 scratch contract as the existing
kernel, including int64 KV indices and sliced unified-attention scratch views.
"""
...
__all__ = [
    "can_use_flashinfer_cutedsl_decode_fullrs",
    "flashinfer_cutedsl_decode_attention_fwd_int2_fullrs",
]
...
def _gemm_rs_k_dequant(
    ...
    rK_atom = cute.make_rmem_tensor((8,), cutlass.BFloat16)
    for k_block in cutlass.range_constexpr(8):
        ...
        for pair_item in cutlass.range_constexpr(4):
            q_lo = cutlass.Int32(
                (word_lo >> (8 * pair_item + field_shift))
                & cutlass.Uint32(0x03)
            )
            q_hi = cutlass.Int32(
                (word_hi >> (8 * pair_item + field_shift))
                & cutlass.Uint32(0x03)
            )
            row_scale = ks0
            row_zero = kz0
            if cutlass.const_expr(pair_item >= 2):
                row_scale = ks1
                row_zero = kz1
            rK_atom[pair_item] = cutlass.BFloat16(
                (cutlass.Float32(q_lo) - row_zero) * row_scale
            )
            rK_atom[pair_item + 4] = cutlass.BFloat16(
                (cutlass.Float32(q_hi) - row_zero) * row_scale
            )
        cute.mma_atom_call(
            atom,
            accumulator_atom,
...
def _gemm_rs_v_dequant_both(
    ...
    rV_lo = cute.make_rmem_tensor((8,), cutlass.BFloat16)
    rV_hi = cute.make_rmem_tensor((8,), cutlass.BFloat16)
    ...
    for k_block in cutlass.range_constexpr(4):
        ...
        vs0 = cutlass.Float32(sVScale[token0])
        vz0 = cutlass.Float32(sVZero[token0])
        vs1 = cutlass.Float32(sVScale[token1])
        vz1 = cutlass.Float32(sVZero[token1])
        vs8 = cutlass.Float32(sVScale[token8])
        vz8 = cutlass.Float32(sVZero[token8])
        vs9 = cutlass.Float32(sVScale[token9])
        vz9 = cutlass.Float32(sVZero[token9])

        # Register order follows Figure 148 of the PTX WGMMA fragment
        # mapping: row0@k[0:2], row1@k[0:2], then k[8:10].
        b00 = cutlass.Int32(sVRaw[token0, packed_dim0])
        b01 = cutlass.Int32(sVRaw[token1, packed_dim0])
        b10 = cutlass.Int32(sVRaw[token0, packed_dim1])
        b11 = cutlass.Int32(sVRaw[token1, packed_dim1])
        b08 = cutlass.Int32(sVRaw[token8, packed_dim0])
        b09 = cutlass.Int32(sVRaw[token9, packed_dim0])
        b18 = cutlass.Int32(sVRaw[token8, packed_dim1])
        b19 = cutlass.Int32(sVRaw[token9, packed_dim1])

        rV_lo[0] = cutlass.BFloat16(
            (cutlass.Float32((b00 >> lo_shift) & 0x03) - vz0) * vs0
        )
        rV_lo[1] = cutlass.BFloat16(
            (cutlass.Float32((b01 >> lo_shift) & 0x03) - vz1) * vs1
        )
        rV_lo[2] = cutlass.BFloat16(
            (cutlass.Float32((b10 >> lo_shift) & 0x03) - vz0) * vs0
        )
        rV_lo[3] = cutlass.BFloat16(
            (cutlass.Float32((b11 >> lo_shift) & 0x03) - vz1) * vs1
        )
        rV_lo[4] = cutlass.BFloat16(
            (cutlass.Float32((b08 >> lo_shift) & 0x03) - vz8) * vs8
        )
        rV_lo[5] = cutlass.BFloat16(
            (cutlass.Float32((b09 >> lo_shift) & 0x03) - vz9) * vs9
        )
        rV_lo[6] = cutlass.BFloat16(
            (cutlass.Float32((b18 >> lo_shift) & 0x03) - vz8) * vs8
        )
        rV_lo[7] = cutlass.BFloat16(
            (cutlass.Float32((b19 >> lo_shift) & 0x03) - vz9) * vs9
        )

        rV_hi[0] = cutlass.BFloat16(
            (cutlass.Float32((b00 >> hi_shift) & 0x03) - vz0) * vs0
        )
        rV_hi[1] = cutlass.BFloat16(
            (cutlass.Float32((b01 >> hi_shift) & 0x03) - vz1) * vs1
        )
        rV_hi[2] = cutlass.BFloat16(
            (cutlass.Float32((b10 >> hi_shift) & 0x03) - vz0) * vs0
        )
        rV_hi[3] = cutlass.BFloat16(
            (cutlass.Float32((b11 >> hi_shift) & 0x03) - vz1) * vs1
        )
        rV_hi[4] = cutlass.BFloat16(
            (cutlass.Float32((b08 >> hi_shift) & 0x03) - vz8) * vs8
        )
        rV_hi[5] = cutlass.BFloat16(
            (cutlass.Float32((b09 >> hi_shift) & 0x03) - vz9) * vs9
        )
        rV_hi[6] = cutlass.BFloat16(
            (cutlass.Float32((b18 >> hi_shift) & 0x03) - vz8) * vs8
        )
        rV_hi[7] = cutlass.BFloat16(
            (cutlass.Float32((b19 >> hi_shift) & 0x03) - vz9) * vs9
        )
...
def _define_decode_kernel(
    ...
    def kernel(
        ...
        sVRaw = smem.allocate_tensor(
            cutlass.Uint8,
            cute.make_layout(
                (block_n, quarter_dim), stride=(quarter_dim, 1)
            ),
            1024,
        )
        sVScale = smem.allocate_tensor(
            cutlass.Float32, cute.make_layout((block_n,)), 16
        )
...
def can_use_flashinfer_cutedsl_decode_fullrs(
    q: torch.Tensor,
...
def flashinfer_cutedsl_decode_attention_fwd_int2_fullrs(
    q: torch.Tensor,
    k_buffer: torch.Tensor,
    v_buffer: torch.Tensor,
    k_scales_zeros: torch.Tensor,
    ...
    """Run the fully register-sourced transposed-WGMMA stage-1 kernel."""

    if not can_use_flashinfer_cutedsl_decode_fullrs(
        q, k_buffer, v_buffer, k_scales_zeros, v_scales_zeros
    ):
        raise ValueError(
